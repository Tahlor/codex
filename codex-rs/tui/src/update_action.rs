...
pub enum UpdateAction {
    /// Update via `npm install -g codexx@latest`.
    NpmGlobalLatest,
    /// Update via `bun install -g codexx@latest`.
    BunGlobalLatest,
    /// Update via `brew upgrade codex`.
    BrewUpgrade,
    /// Update via the Codexx standalone Unix installer.
    StandaloneUnix,
    /// Update via the Codexx standalone Windows installer.
    StandaloneWindows,
}
...
impl UpdateAction {
    ...
    pub fn command_args(self) -> (&'static str, &'static [&'static str]) {
        match self {
            UpdateAction::NpmGlobalLatest => ("npm", &["install", "-g", "codexx@latest"]),
            UpdateAction::BunGlobalLatest => ("bun", &["install", "-g", "codexx@latest"]),
            UpdateAction::BrewUpgrade => ("brew", &["upgrade", "--cask", "codex"]),
            UpdateAction::StandaloneUnix => (
                "sh",
                &[
                    "-c",
                    "curl -fsSL https://raw.githubusercontent.com/Tahlor/codex/main/scripts/install/install.sh | sh",
                ],
            ),
            UpdateAction::StandaloneWindows => (
                ...
                &[
                    "-c",
                    "irm https://raw.githubusercontent.com/Tahlor/codex/main/scripts/install/install.ps1|iex",
                ],
            ),
        }
    }
