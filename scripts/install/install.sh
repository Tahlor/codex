...
ALIAS_COMMANDS="codexxx codex-v5 codex-v6 codex-v7 codexx-v5 codexx-v6 codexx-v7"
...
GITHUB_REPO="${CODEXX_RELEASE_REPO:-Tahlor/codex}"
...
file_sha256() {
  ...
  echo "sha256sum, shasum, or openssl is required to verify the Codexx download." >&2
  exit 1
}
...
verify_archive_digest() {
  ...
  if [ "$actual_digest" != "$expected_digest" ]; then
    echo "Downloaded Codexx archive checksum did not match release metadata." >&2
...
require_command() {
  if ! command -v "$1" >/dev/null 2>&1; then
    echo "$1 is required to install Codexx." >&2
    exit 1
  fi
}
...
resolve_version() {
  ...
  if [ -z "$resolved" ]; then
    echo "Failed to resolve the latest Codexx release version." >&2
    exit 1
  fi
...
add_to_path() {
  ...
  begin_marker="# >>> Codexx installer >>>"
  end_marker="# <<< Codexx installer <<<"
...
cleanup_stale_install_artifacts() {
  ...
  if [ -d "$BIN_DIR" ]; then
    for command_name in $PRIMARY_COMMAND $ALIAS_COMMANDS; do
...
current_installed_version() {
  version="$(version_from_binary "$CURRENT_LINK/$PRIMARY_COMMAND" || true)"
  if [ -n "$version" ]; then
    printf '%s\n' "$version"
    return 0
  fi
...
resolve_existing_codexx() {
  command -v "$PRIMARY_COMMAND" 2>/dev/null || true
}
...
print_launch_instructions() {
  case "$path_action" in
    added)
      step "Current terminal: export PATH=\"$BIN_DIR:\$PATH\" && $PRIMARY_COMMAND"
      step "Future terminals: open a new terminal and run: $PRIMARY_COMMAND"
    ...
    configured)
      ...
      ;;
    *)
      step "Current terminal: $PRIMARY_COMMAND"
      step "Future terminals: open a new terminal and run: $PRIMARY_COMMAND"
      ;;
  esac
}
...
maybe_launch_codex_now() {
  if prompt_yes_no "Start Codexx now?"; then
    step "Launching Codexx"
    "$BIN_PATH"
  fi
}

detect_conflicting_install() {
  existing_path="$(resolve_existing_codexx)"
  ...
  step "Detected existing $manager-managed Codexx at $existing_path"
  warn "Multiple managed Codexx installs can be ambiguous because PATH order decides which one runs."
}
...
handle_conflicting_install() {
  ...
  case "$conflict_manager" in
    brew)
      uninstall_cmd="brew uninstall --cask codexx"
      ;;
    bun)
      uninstall_cmd="bun remove -g codexx"
      ;;
    *)
      uninstall_cmd="npm uninstall -g codexx"
      ;;
  esac

  if prompt_yes_no "Uninstall the existing $conflict_manager-managed Codexx now?"; then
    step "Running: $uninstall_cmd"
    if ! sh -c "$uninstall_cmd"; then
      warn "Failed to uninstall the existing $conflict_manager-managed Codexx. Continuing with the standalone install."
    fi
  else
    warn "Leaving the existing $conflict_manager-managed Codexx installed. PATH order will determine which codexx runs."
  fi
}
...
install_release() {
  ...
  cp "$vendor_root/codex/codex" "$stage_release/$PRIMARY_COMMAND"
  for alias_command in $ALIAS_COMMANDS; do
    cp "$vendor_root/codex/codex" "$stage_release/$alias_command"
  done
  cp "$vendor_root/path/rg" "$stage_release/codex-resources/rg"
  chmod 0755 "$stage_release/$PRIMARY_COMMAND"
  for alias_command in $ALIAS_COMMANDS; do
    chmod 0755 "$stage_release/$alias_command"
  done
...
update_visible_command() {
  ...
  replace_path_with_symlink "$BIN_DIR/$command_name" "$CURRENT_LINK/$command_name" "$tmp_link"
}

update_visible_commands() {
  update_visible_command "$PRIMARY_COMMAND"
  for alias_command in $ALIAS_COMMANDS; do
    update_visible_command "$alias_command"
  done
}
...
if [ -n "$current_version" ] && [ "$current_version" != "$resolved_version" ]; then
  step "Updating Codexx CLI from $current_version to $resolved_version"
elif [ -n "$current_version" ]; then
  step "Updating Codexx CLI"
else
  step "Installing Codexx CLI"
fi
...
printf 'Codexx CLI %s installed successfully.\n' "$resolved_version"
maybe_launch_codex_now
